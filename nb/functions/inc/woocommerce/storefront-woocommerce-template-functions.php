...
if ( ! function_exists( 'storefront_all_products' ) ) {
	/**
	 ...
	 * @return void
	 */
	function storefront_all_products( $args ) {
		$args = apply_filters(
			'storefront_all_products_args', array(
				'limit'      => 9999,
				'columns'    => 4,
				'orderby'    => 'date',
				'order'      => 'desc',
		...
		$shortcode_content = storefront_do_shortcode(
			'products', apply_filters(
				'storefront_all_products_shortcode_args', array(
					'per_page'   => intval( $args['limit'] ),
					'columns'    => intval( $args['columns'] ),
					'orderby'    => esc_attr( $args['orderby'] ),
					'order'      => esc_attr( $args['order'] ),
		 ...
		 */
		if ( false !== strpos( $shortcode_content, 'product' ) ) {
			echo '<section class="storefront-product-section " aria-label="' . esc_attr__( 'All products', 'storefront' ) . '">';

			do_action( 'storefront_homepage_before_all_products' );

			echo '<h2 class="section-title">' . wp_kses_post( $args['title'] ) . '</h2>';

			do_action( 'storefront_homepage_after_all_products_title' );

			echo $shortcode_content; // WPCS: XSS ok.

			do_action( 'storefront_homepage_after_all_products' );
